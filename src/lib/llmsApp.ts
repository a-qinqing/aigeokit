...
function setBusy(busy: boolean): void {
  ...
  ($('llms-import-url') as HTMLButtonElement).disabled = busy;
}
...
async function fetchSitemap(url: string): Promise<{ ok: boolean; message: string; xml?: string; truncated?: boolean }> {
  ...
  try {
    ...
    return { ok: false, message: 'Sitemap fetch failed — check the URL, or paste the XML instead.' };
  }
  if (!payload.ok) {
    const titles: Record<string, string> = {
      'invalid-url': 'That URL is not allowed — use a public http(s) sitemap URL.',
      'not-found': 'No sitemap found at that URL (HTTP 404/410).',
      'not-a-sitemap': 'That URL did not return an XML sitemap — it should point to a <urlset> document such as …/sitemap.xml.',
      timeout: 'The site did not respond in time — try pasting the sitemap XML instead.',
      unreachable: 'Could not reach that host — try pasting the sitemap XML instead.',
    };
    return { ok: false, message: titles[payload.error ?? ''] ?? payload.message };
  }
  return { ok: true, message: '', xml: payload.xml_text, truncated: payload.truncated };
}
